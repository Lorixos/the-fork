...
function normalizeRow(row) {
  const rawCreative = firstValue(row, ["creative", "creative_name", "ad_name", "asset_name", "adset_name"]);
  return {
    ...
    rawCreative: rawCreative,
    creative: cleanTikTokAdName(rawCreative),
...
async function saveCommentary() {
  ...
  try {
    const payload = {
      ...
      author: state.currentUser ? state.currentUser.email : state.commentaryAuthor,
...
async function saveBudgets() {
  ...
  try {
    const payload = {
      ...
      updated_by: state.currentUser ? state.currentUser.email : "user"
    };
